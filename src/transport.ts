...
//   2. POST /ask — sends the finalized question, gets back a RAG-backed
//      answer generated by the Worker via AI Search + Claude.
//   3. POST /suggest — legacy Cue endpoint; retained for compatibility.
//
...
export interface CueTransport {
  ...
  requestAsk: (params: {
    ...
    ok: true
    answer: string
    chunks?: Array<{ source: string; score: number }>
...
export function createTransport(workerUrl: string, bearerToken: string): CueTransport {
  ...
  return {
    ...
    async requestAsk({ mode, transcript, conditions }) {
      if (!ready) return { ok: false as const, error: 'Worker not configured' }
      const ctrl = new AbortController()
      const timer = setTimeout(() => ctrl.abort(), 60_000)
      try {
        const resp = await fetch(`${baseHttp}/ask`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${bearerToken}`,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ mode, transcript, conditions }),
          signal: ctrl.signal,
        })
        if (!resp.ok) {
          const txt = await resp.text().catch(() => '')
          return { ok: false as const, error: explainHttp(resp.status, txt) }
        }
        const json = (await resp.json()) as
          | { ok: true; answer: string; chunks?: Array<{ source: string; score: number }> }
          | { ok: false; error: string }
        return json
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err)
